...
class AppStore extends Component {
  ...
  render() {
    ...
    const filteredList = appsList.filter(each => each.category === category)
    const finalFilteredList = filteredList.filter(each => {
      const value = each.appName.toLowerCase()
      ...
      <div className="mainContainer">
        <div className="insideContainer">
          ...
          <div className="appContainer">
            <ul className="appUnordered">
              {finalFilteredList.map(each => (
                <AppObject item={each} key={each.appId} />
              ))}
            </ul>
          </div>
        </div>
